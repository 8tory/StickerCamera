...
public final class DataUtils {
    ...
    public static <T extends Object> T getObject(Context context, Class<T> c, String name) {
        T t = null;
        try {
            String str = DataUtils.getStringPreferences(context, name);
            if (StringUtils.isNotBlank(str)) {
                t = (T) JSON.parseObject(str, c);
            }
        } catch (Exception e) {
            Log.e("DataUtils", "解析信息失败");
            DataUtils.setStringPreferences(context, name, "");
        }
        return t;
    }
